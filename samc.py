...
class SAMCParameters(object):
	...
	# psi - Relative weights of sampled points
	psi = attr.ib(default=lambda x: 1)
...
def SAMC(samc_parameters):
	...
	for n_iter in tnrange(_p.n_iterations):
		...
		if _p.fix_theta:
			r = exp(_p.theta0[current_partition] - _p.theta0[suggested_partition]) * _p.psi(y) / _p.psi(current_x) * proposal_ratio
		else:
			r = exp(current_theta[current_partition] - current_theta[suggested_partition]) * _p.psi(y) / _p.psi(current_x) * proposal_ratio
		...
		statistics.append([updated, current_partition, current_statistic, current_theta.copy(), None])
		...
		if n_iter % (_p.n_iterations/10) == 1:
			tqdm.write("RSE: %f\t Chi^2 p, log10(p): %f %f\t LinReg p, log10(p): %f %f" % (relative_sampling_error, chisquare.pvalue, log10(chisquare.pvalue), regp, log10(regp)))
...
def SAMC_two_sample_t_test(n_partitions, n_iterations, n=100, replace_proportion=0.05):
	...
	return SAMC_simple(SAMCParameters(
		x0=x0, 
